...
export default function FinanceCard() {
  ...
  useEffect(() => {
    async function connectPokerFactoryContract() {
      ...
      const pokerFactoryContractEvents =
        await connectedPokerFactoryContract.queryFilter("PokerFinanceCreated");
      ...
      for (let i = 0; i < pokerFactoryContractEvents.length; i++) {
        ...
        addressPlayerArray.push(
          pokerFactoryContractEvents && pokerFactoryContractEvents[i].args
            ? pokerFactoryContractEvents[i].args[0]
            : ""
        );
        amountTotalArray.push(
          pokerFactoryContractEvents && pokerFactoryContractEvents[i].args
            ? pokerFactoryContractEvents[i].args[2]
            : 0
        );
        feeArray.push(
          pokerFactoryContractEvents && pokerFactoryContractEvents[i].args
            ? pokerFactoryContractEvents[i].args[3]
            : 0
        );
        datetimeLimitArray.push(
          pokerFactoryContractEvents && pokerFactoryContractEvents[i].args
            ? Number(pokerFactoryContractEvents[i].args[4])
            : 0
        );
      }
